...
function queryToObj(queryStr) {
  ...
  try {
    for (var _iterator = queryArr[Symbol.iterator](), _step; !(_iteratorNormalCompletion = (_step = _iterator.next()).done); _iteratorNormalCompletion = true) {
      var i = _step.value;
      var key = i.split('=')[0];
      var val = i.split('=')[1];
      result[key] = val;
...
Mock.mock('/addStuByForm/', 'post', function (option) {
  var newStu = queryToObj(decodeURIComponent(option.body)); // 这里正常的逻辑，应该是连接数据库进行存储，返回新增成功的信息
...
Mock.mock('/getOneStuInfo/', 'post', function (option) {
  //获取客户端传递过来的id
  var id = queryToObj(decodeURIComponent(option.body)).id; // 这里正常的逻辑，应该连接后端数据库，获取对应 id 的学生数据
...
Mock.mock('/editStuByForm/', 'post', function (option) {
  var stuInfo = queryToObj(decodeURIComponent(option.body)); // 这里正常的逻辑，连接数据库，将新的信息覆盖旧的信息
...
Mock.mock(RegExp('/delStu/?[\w\W]*'), 'delete', function (option) {
  var id = option.url.split("/")[2]; // 这里正常的逻辑，就是应该连接数据库，删除和这个 id 相对应的数据
...
Mock.mock('/searchStu/', 'post', function (option) {
  var stuInfo = queryToObj(decodeURIComponent(option.body)); // 这里正常逻辑，就应该连接数据库，根据上面 searchInfo 的条件来搜索数据，然后返回给客户端
